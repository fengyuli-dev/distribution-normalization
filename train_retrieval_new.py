...
import sys
CLIP_TRAINING_DIR = '/share/cuvl/image_caption_metrics/clip-training'
sys.path.insert(1, CLIP_TRAINING_DIR)
...
# both are same but AdamW has a default weight decay
...
DATA_CONFIG_PATH = Path(CLIP_TRAINING_DIR, 'dataloader/data_config.yaml')
TRAINER_CONFIG_PATH = Path(CLIP_TRAINING_DIR, 'trainer/train_config.yaml')
MODEL_CONFIG_PATH = Path(CLIP_TRAINING_DIR, 'model/model_config.yaml')


@progress_alerts(func_name="train retrieval")
def train(config, train_dataset, model):
    ...
    optimizer = AdamW(model.parameters(), lr=config.optimizer.params.lr,
    ...
    for epoch in trange(int(config.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            image_features = model.module.get_image_features(input_images)
            text_features = model.module.get_text_features(input_texts)
            ...
            labels = torch.arange(len(logits_per_image)).to(
            ...
            if (step + 1) % config.gradient_accumulation_steps == 0:
                ...
                if config.n_gpu == 1:
                    ...
                    model.module.logit_scale.data = torch.clamp(
                        model.module.logit_scale.data, 0, 4.6052)
                ...
                #         global_step == t_total:
                #     # saving checkpoint
                #     save_checkpoint(config, epoch, global_step,
                #                     model, optimizer)
        if (config.save_epochs > 0 and epoch % config.save_epochs == 0):
            # saving checkpoint
            save_checkpoint(config, epoch, global_step,
                            model, optimizer)
...
def save_checkpoint(config, epoch, global_step, model, optimizer):
    ...
    checkpoint_path = os.path.join(
        config.saved_checkpoints, f'checkpoint_{epoch}_{global_step}.pt')
    save_num = 0
    while (save_num < 10):
        try:
...
def main():
    ...
    parser.add_argument("--train_img_dir", default=None, type=str, required=False,
                        help="path of directory containing COCO training images")
    parser.add_argument("--train_annotation_file", default=None,
                        type=str, required=False, help="path of COCO annotation file")
    ...
    logger = setup_logger("CLIP_COCO_TRAIN", config.logs,
                          0, filename="training_logs.txt")
    ...
    train_dataset = CLIP_COCO_dataset(config)
    ...
    # torch.save(model, 'cocoCLIP150.pt')
